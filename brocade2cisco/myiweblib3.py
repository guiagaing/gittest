...
def filecreation(deviceType,hostname, filenamePrefix,filenameSuffix,config,interface_ve_dict):
    #print "alist"
    line = ''
    vlan = ''
    vlanDescription = ''
    		...
    		for item in range (len(config)):
    			for k in range(len(config[item])):
        			...
        			if re.search('^interface',line):
        				...
        				vlanID = line.strip('interface').strip().strip('vlan').strip()
        				ivlanID = (int)(vlanID)
        				vlanDescription = interface_ve_dict[ivlanID]['vlanName']
        				d.write(vlan + '\n')
        				d.write('name' + ' ')
        				d.write(vlanDescription + '\n')
        				d.write('exit' + '\n')
        				d.write('\n')
...
def Generate_Interface_Ve_Dict(vlan_list):
	...
	vlan_list = map(int, vlan_list)
	
	inter_ve_dict = {}
	for vlan in vlan_list:
		inter_ve_dict.update({vlan:{"vlanName":'',"description":'',"ipv4":[], "ipv6":[],"vip-v4":[],"vip-v6":[],"ip-helper":[],"mtu": '-', "ip-mtu": '-', "vrf": '',"access-group-in": '',"access-group-out": '',"ve_type_v4": '-',"ve_type_v6":'-',"enable":1}})
	return inter_ve_dict

#########################################################

def PrepareCiscoInterfaceVe(vlan,brocadeInterfaceVe,hostname):
	nbreIpv4 = 0
	nbreIpv6 = 0
	nbreVipV4 = 0
	nbreVipV6 = 0
	...
	if brocadeInterfaceVe['ve_type_v4'] != 'NON-VALIDE':
		ciscoInterface.append('interface vlan' + ' ' + (str)(vlan) )
		if len(brocadeInterfaceVe['description']) != 0:
			ciscoInterface.append('description' + ' ' + brocadeInterfaceVe['description'] )
...
# V4 Auswertung			
		...
		for k in range(nbreVipV4):	
			if((nbreVipV4 != 0) and ( k == 0)):
				...
				ciscoInterface.append('standby ' + str(standbygroup) +' ' + 'priority ' + priority)
				ciscoInterface.append('standby ' + str(standbygroup) +' ' + 'preempt delay minimum 60')
				ciscoInterface.append('standby ' + str(standbygroup) +' ' + 'authentication VLAN' + (str)(vlan))
...
def GenerateCiscoConfig(ve_type,InteVeModel,hostname):
	...
	for k,v in InteVeModel.iteritems():
		...
		if InteVeModel[k]['ve_type_v4'] == ve_type:	
			ciscoInterface.append((PrepareCiscoInterfaceVe(k,InteVeModel[k],hostname)))
...
def GenerateBrocadeConfig(ve_type,InteVeModel):
	...
	for k,v in InteVeModel.iteritems():
		#if InteVeModel[k]['ve_type_v4'] != 'NON-VALIDE':
		if InteVeModel[k]['ve_type_v4'] == 'NON-VALIDE':
			brocadeInterface.append('no interface ve ' + (str)(k))
			brocadeInterface.append('\n')

		elif InteVeModel[k]['ve_type_v4'] == ve_type:	
			brocadeInterface.append('interface ve ' + (str)(k))
			brocadeInterface.append('\n')
